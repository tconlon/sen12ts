...
import pandas as pd
# from convert_tif_to_tfrecord_by_dltile import *
...
def predict_timeseries(args, generator, checkpoint_folder, test_funcs):
    '''
    Group images by the DL tile. Create a tf.data.Dataset for each DLTile's worth
    of imagery. Predict NDVI for each tile's ds, and present results. 
    
    '''
    # Define save lcoations and parameters
    overwrite_existing_dltile_ds = True
    folder_name = f'../../data/processed/s1_timeseries_prediction_images/{args.TIMESERIES_PRED_REGION}/test'
    ts_save_folder = f'../../reports/timeseries_predictions/{checkpoint_folder}'
    if not os.path.exists(ts_save_folder):
        os.makedirs(ts_save_folder)
    
    # Find all the timeseries tifs + unique dl tiles
    input_directory = f'{folder_name}/input_tifs'
    file_names = glob.glob(f'{input_directory}/*.tif')
    unique_dltiles = np.unique([i.split('/')[-1].split('_')[1] for i in file_names])

    # Loop through the unique DLtiles to present results per tile                           
    for dltile in unique_dltiles:
        
        # Find all tifs for the tile
        all_tifs_for_tile = sorted([i for i in file_names if dltile in i])
        
        # Create a .tfrecord outfile for the select tifs
        out_file = f'{folder_name}/input_datasets/dltile_{dltile}_nimgs_'\
                   f'{len(all_tifs_for_tile)}.tfrecord'
        if not os.path.exists(f'{folder_name}/input_datasets/'):
            os.makedirs(f'{folder_name}/input_datasets')
        
        # If desired, create a new .tfrecord
        if not os.path.exists(out_file) or overwrite_existing_dltile_ds:
            make_s1s2_tfrecord(args, all_tifs_for_tile, out_file)
        
        # Load dataset
        test_dataset = tf.data.TFRecordDataset(out_file).map(
        ...
        predicted_ndvi_array = np.zeros((args.IMG_HEIGHT, args.IMG_WIDTH, 
                                         len(all_tifs_for_tile)))
        actual_ndvi_array = np.zeros((args.IMG_HEIGHT, args.IMG_WIDTH, 
                                         len(all_tifs_for_tile) + 1))
        s1_date_list = []
        
        
        # Predict NDVI for test dataset
        for ix, (test_input, test_target, test_s1_date) in test_dataset.enumerate():
            prediction  = generator(test_input, training=False)
                        
            # Save CDL to last layer in NDVI array
            if ix == 0:
                actual_ndvi_array[..., -1] = test_target[0, ..., -1]
            
            # Save predicted and actual NDVIs
            predicted_ndvi_array[..., ix] = prediction[0, ..., 0]
            actual_ndvi_array[..., ix]    = test_target[0, ..., 0]
            s1_date_list.append(test_s1_date.numpy()[0].decode('utf-8'))
        
        # Process the results
        crop_means_dict = timeseries_analysis(dltile, predicted_ndvi_array, 
                                              actual_ndvi_array, s1_date_list,
                                              checkpoint_folder)
        # Clean the processed results
        metrics_dict = process_crop_means_dict(crop_means_dict)  
        
        # Plot
        plotting_timeseries_results(metrics_dict, dltile, checkpoint_folder, ts_save_folder)
